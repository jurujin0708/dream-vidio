...
중요: narration은 반드시 "안녕, 나는 미래의 {student['name']}야." 처럼
미래의 자신이 직접 말하는 1인칭 자기소개 형식으로 작성해주세요.
각 장면마다 그 시점의 자신이 현재의 자신에게 말을 거는 방식으로요.

반드시 JSON만 응답. 다른 텍스트 없이.
{{"title":"15자이내","tagline":"25자이내","scenes":[
  {{"scene_number":1,"time_label":"현재 · 중학교 2학년","narration":"미래의 내가 현재를 돌아보며 1인칭으로 40자이내","visual_desc":"30자이내"}},
  {{"scene_number":2,"time_label":"5년 후 · 고등학교 졸업","narration":"5년후의 내가 직접 말하는 1인칭 40자이내","visual_desc":"30자이내"}},
  {{"scene_number":3,"time_label":"10년 후 · 꿈을 향해","narration":"10년후의 내가 직접 말하는 1인칭 40자이내","visual_desc":"30자이내"}},
  {{"scene_number":4,"time_label":"15년 후 · {student['dream_job']}이 되다","narration":"꿈을 이룬 내가 직접 말하는 1인칭 40자이내","visual_desc":"30자이내"}},
  {{"scene_number":5,"time_label":"오늘 · 현재의 너에게","narration":"미래의 내가 현재의 나에게 응원 메시지 1인칭 40자이내","visual_desc":"30자이내"}}
]}}"""
...
def make_tts_audio(text: str, out_path: Path) -> bool:
    """gTTS로 한국어 나레이션 생성"""
    try:
...
def download_bgm(tmp_dir: Path) -> Path | None:
    """저작권 무료 배경음악 다운로드"""
    import urllib.request
    bgm_urls = [
        ("https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808fbf07a.mp3", "inspiring.mp3"),
        ("https://cdn.pixabay.com/download/audio/2021/11/25/audio_cb31e1c5a2.mp3", "dream.mp3"),
        ("https://cdn.pixabay.com/download/audio/2022/03/10/audio_c8c8a73467.mp3", "hopeful.mp3"),
    ]
...
def build_video_bytes(frames: list, font_path, story: dict = None) -> bytes:
    """PIL 프레임 리스트 → mp4 bytes (나레이션 + 배경음악 포함)"""
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        clips = []
        total_duration = len(frames) * SCENE_DUR

        # 영상 클립 생성
        for i, frm in enumerate(frames):
            p = tmp / f"f{i:03d}.jpg"
            frm.save(str(p), quality=92)
            clip = ImageClip(str(p), duration=SCENE_DUR)
        ...
        if story:
            # 타이틀 나레이션
            title_tts = tmp / "tts_title.mp3"
            make_tts_audio(f"{story['tagline']}", title_tts)

            # 각 장면 나레이션
            for i, sc in enumerate(story.get("scenes", [])):
                tts_path = tmp / f"tts_{i}.mp3"
                if make_tts_audio(sc.get("narration",""), tts_path):
                    try:
                        a = AudioFileClip(str(tts_path))
                        # 장면 시작 시간에 맞게 배치 (타이틀=0, 장면1=6초, ...)
                        start_t = (i + 1) * SCENE_DUR + 0.5
                        narration_clips.append(a.set_start(start_t))
                    except Exception:
                        pass

        # 배경음악
        try:
            bgm_path = download_bgm(tmp)
            if bgm_path:
                bgm = AudioFileClip(str(bgm_path))
                if bgm.duration < total_duration:
                    bgm = audio_loop(bgm, duration=total_duration)
                else:
                    bgm = bgm.subclip(0, total_duration)
                # 나레이션 있으면 음악 볼륨 낮추기 (20%), 없으면 40%
                vol = 0.2 if narration_clips else 0.4
                bgm = bgm.volumex(vol).audio_fadeout(2.0)

                if narration_clips:
                    # 나레이션 + 배경음악 합치기
                    all_audio = CompositeAudioClip([bgm] + narration_clips)
                else:
                    all_audio = bgm

                final = final.set_audio(all_audio)
        except Exception:
            # 배경음악 실패해도 나레이션만이라도 추가
            if narration_clips:
                try:
                    all_audio = CompositeAudioClip(narration_clips)
                    final = final.set_audio(all_audio)
                except Exception:
                    pass
...
if generate_btn and ready:
    ...
    try:
        ...
        # 4. 영상 합성 + 나레이션 + 배경음악
        status.markdown("🗣️ **AI 나레이션 생성 중...**")
        progress.progress(90)
        status.markdown("🎬 **영상·나레이션·음악을 합치고 있어요... (1~2분 소요)**")
        video_bytes = build_video_bytes(frames, font_path, story=story)
        ...
        # 결과
        st.balloons()
        st.success(f"🎉 {name}의 꿈 영상이 완성됐어요! 🎵 배경음악도 포함됐어요!")
        ...
        st.markdown("---")
        st.download_button(
            label="⬇️  영상 다운로드 (.mp4)  — 배경음악 포함",
            data=video_bytes,
            file_name=f"dream_{safe}.mp4",
            mime="video/mp4",
            use_container_width=True,
        )
        st.caption("💡 다운로드한 영상을 카카오톡, 유튜브, 학급 SNS에 공유해보세요!")
